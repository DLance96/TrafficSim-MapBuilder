...
class road(object):
    ...
    def __init__(self, start_coord, length, out_lanes, in_lanes):
        ...
        self.end_coord = coordinates.coordinates(start_coord.get_x() + length, start_coord.get_y())
    ...
    def is_on_road(self, coordinate):
        if (coordinate.x > self.coord.x) & (coordinate.x < self.end_coord.x):
            if coordinate.y > (self.coord.get_y() - (self.in_lanes * 20)):
                if coordinate.y < (self.coord.get_y() + (self.out_lanes * 20)):
                    return True
        return False
    ...
    def get_points(self):
        points = []
        points.append(coordinates.coordinates(self.coord.get_x(), self.coord.get_y() - (self.in_lanes * 20)))
        points.append(coordinates.coordinates(self.coord.get_x(), self.coord.get_y() + (self.out_lanes * 20)))
        points.append(coordinates.coordinates(self.end_coord.get_x(), self.end_coord.get_y() + (self.out_lanes * 20)))
        points.append(coordinates.coordinates(self.end_coord.get_x(), self.end_coord.get_y() - (self.in_lanes * 20)))
        return points
